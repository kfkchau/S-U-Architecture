...
**The human sits on top.** Three decisions never go to the machine: what are we making (definition), where do the thresholds sit (calibration), is the result accepted (acceptance). This is the executive pattern made structural: leadership does not make every decision; it defines the work, sets the standards, and reviews the result. In reasoning-tier terms: pattern-level WHAT work runs as code, method-level HOW work runs inside forms, and the WHY tier stays human. The boundary is not caution about today's models. Most of the knowledge that governs real organisations is unstructured judgment; AI can now work that layer, but its work there is judgment, and ungoverned judgment is where every ungated autonomous system eventually fails. The frontier's own results agree: agents given real autonomy over their own pipelines reward-hack.
